...
export class EmailTemplateManager {
    ...
    registerHelpers() {
        ...
        Handlebars.registerHelper('formatBlock', (block) => {
            switch (block.type) {
                case 'paragraph':
                    return new Handlebars.SafeString(`<p class="email-paragraph ${block.metadata?.className || ''}"
                            style="${block.metadata?.style || ''}">${block.content}</p>`);
                case 'bulletList':
                    return new Handlebars.SafeString(`<ul class="email-list ${block.metadata?.className || ''}"
                            style="${block.metadata?.style || ''}">
                            ${Array.isArray(block.content)
                        ? block.content.map(item => `<li class="email-list-item">${item}</li>`).join('')
                        : `<li class="email-list-item">${block.content}</li>`}
                        </ul>`);
                case 'heading':
                    return new Handlebars.SafeString(`<h2 class="email-heading ${block.metadata?.className || ''}"
                            style="${block.metadata?.style || ''}">${block.content}</h2>`);
                case 'signature':
                    return new Handlebars.SafeString(`<div class="email-signature">${block.content}</div>`);
                case 'callout':
                    return new Handlebars.SafeString(`<div class="email-callout ${block.metadata?.className || ''}"
        ...
        Handlebars.registerHelper('priorityBadge', (priority) => {
            ...
            return new Handlebars.SafeString(`<div class="priority-badge"
    ...
    formatBlock(block) {
        switch (block.type) {
            ...
            case 'bulletList':
                if (!block.content)
                    return ''; // Ensure content is available
    ...
    validateTemplate(template) {
        if (!template.id || !template.html || !template.variables) {
            throw new Error('Invalid template: missing required fields (id, html, variables)');
        }
        // Validate HTML structure
        if (!template.html.includes('{{content}}') &&
            !template.html.includes('{{blocks}}')) {
            throw new Error('Invalid template: missing required content placeholder');
        }
    }
}
