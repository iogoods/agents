// src/actions/solana/getTokenPairs.ts
import {
  composeContext,
  elizaLogger,
...
// src/environment.ts
var moralisEnvSchema = z.object({
  MORALIS_API_KEY: z.string().min(1, "Moralis API key is required")
});
async function validateMoralisConfig(runtime) {
  try {
    const config = {
...
// src/templates/tokenPairs.ts
var getTokenPairsTemplate = `Given the most recent message only, extract the Solana token address that we need to fetch pairs for. This is specifically for Solana blockchain only.
...
var SOLANA_API_BASE_URL = "https://solana-gateway.moralis.io";
var API_ENDPOINTS = {
  SOLANA: {
    TOKEN_PAIRS: (tokenAddress) => `/token/mainnet/${tokenAddress}/pairs`,
    PAIR_STATS: (pairAddress) => `/token/mainnet/pairs/${pairAddress}/stats`,
    PAIR_OHLCV: (pairAddress) => `/token/mainnet/pairs/${pairAddress}/ohlcv`,
    TOKEN_STATS: (tokenAddress) => `/token/mainnet/${tokenAddress}/pairs/stats`,
    TOKEN_PRICE: (tokenAddress) => `/token/mainnet/${tokenAddress}/price`,
    TOKEN_METADATA: (tokenAddress) => `/token/mainnet/${tokenAddress}/metadata`
  }
};

// src/actions/solana/getTokenPairs.ts
var getTokenPairs_default = {
  name: "GET_SOLANA_TOKEN_PAIRS",
  // Changed name to be Solana-specific
  similes: [
    "FETCH_SOLANA_PAIRS",
    "LIST_SOLANA_PAIRS",
    "SHOW_SOLANA_PAIRS",
  ...
  handler: async (runtime, message, state, _options, callback) => {
    ...
    try {
      ...
      const topPairs = response.data.pairs.sort((a, b) => b.liquidityUsd - a.liquidityUsd).slice(0, 10);
      elizaLogger.success(
      ...
      if (callback) {
        ...
        const formattedPairs = pairsInfo.map(
          (pair) => `- ${pair.pairLabel} (${pair.pairAddress})
...
// src/actions/solana/getPairStats.ts
import {
...
// src/templates/pairStats.ts
var getPairStatsTemplate = `Given the most recent message only, extract the Solana pair address that we need to fetch stats for. This is specifically for Solana blockchain only.
...
var getPairStats_default = {
  ...
  description: "Get detailed statistics for a specific trading pair on Solana blockchain",
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger2.log("Starting Moralis GET_SOLANA_PAIR_STATS handler...");
    let currentState;
    if (!state) {
      currentState = await runtime.composeState(message);
    } else {
    ...
    try {
      elizaLogger2.log("Composing pair stats context...");
      const statsContext = composeContext2({
        state: currentState,
        template: getPairStatsTemplate
      });
      elizaLogger2.log("Extracting pair address...");
      ...
      if (callback) {
        const formattedStats = `Pair: ${stats.pairLabel} (${stats.pairAddress})
...
${formattedStats}`,
      ...
      elizaLogger2.error("Error in GET_SOLANA_PAIR_STATS handler:", error);
      const errorMessage = error instanceof Error ? error.message : String(error);
      if (callback) {
        callback({
...
// src/actions/solana/getPairOHLCV.ts
import {
...
// src/templates/pairOHLCV.ts
var getPairOHLCVTemplate = `Given the most recent message only, extract information needed to fetch OHLCV (price history) data for a Solana trading pair. This is specifically for Solana blockchain only.
...
- fromDate: start date in YYYY-MM-DD format (if not specified or if message says "past X days", calculate from current date ${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]})
- toDate: end date in YYYY-MM-DD format (if not specified, use current date ${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]})
...
var getPairOHLCV_default = {
  ...
  description: "Get OHLCV (price history) data for a specific trading pair on Solana blockchain",
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger3.log("Starting Moralis GET_SOLANA_PAIR_OHLCV handler...");
    let currentState;
    if (!state) {
      currentState = await runtime.composeState(message);
    } else {
    ...
    try {
      elizaLogger3.log("Composing OHLCV request context...");
      const ohlcvContext = composeContext3({
        state: currentState,
        template: getPairOHLCVTemplate
      });
      elizaLogger3.log("Extracting OHLCV parameters...");
      ...
      elizaLogger3.log("API Response:", response.data);
      ...
      if (callback) {
        ...
        let summaryText = `Here's the ${timeframe} price history in ${currency} for the Solana trading pair from ${params.fromDate} to ${params.toDate}:
...
`;
        ...
        const periodReturn = (lastCandle.close - firstCandle.open) / firstCandle.open * 100;
...
`;
        for (const candle of sortedCandles.slice(0, displayCount)) {
          ...
          summaryText += `
Time: ${date}
Open: $${candle.open.toFixed(4)}, High: $${candle.high.toFixed(4)}, Low: $${candle.low.toFixed(4)}, Close: $${candle.close.toFixed(4)}
Volume: $${Math.round(candle.volume).toLocaleString()}, Trades: ${candle.trades.toLocaleString()}`;
        }
        callback({
          text: summaryText,
          content: {
      ...
      elizaLogger3.error("Error in GET_SOLANA_PAIR_OHLCV handler:", error);
      const errorMessage = error instanceof Error ? error.message : String(error);
      if (callback) {
        callback({
...
// src/actions/solana/getTokenStats.ts
import {
...
// src/templates/tokenStats.ts
var getTokenStatsTemplate = `Given the most recent message only, extract the Solana token address that we need to fetch aggregated stats for. This is specifically for Solana blockchain only.
...
var getTokenStats_default = {
  ...
  description: "Get aggregated statistics across all pairs for a specific token on Solana blockchain",
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger4.log("Starting Moralis GET_SOLANA_TOKEN_STATS handler...");
    let currentState;
    if (!state) {
      currentState = await runtime.composeState(message);
    } else {
    ...
    try {
      elizaLogger4.log("Composing token stats context...");
      const statsContext = composeContext4({
        state: currentState,
        template: getTokenStatsTemplate
      });
      elizaLogger4.log("Extracting token address...");
...
// src/actions/solana/getTokenPrice.ts
import {
...
// src/templates/tokenPrice.ts
var getTokenPriceTemplate = `Given the most recent message only, extract the Solana token address that we need to fetch price for. This is specifically for Solana blockchain only.
...
var getTokenPrice_default = {
  ...
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger5.log("Starting Moralis GET_SOLANA_TOKEN_PRICE handler...");
    let currentState;
    if (!state) {
      currentState = await runtime.composeState(message);
    } else {
    ...
    try {
      ...
      const priceContext = composeContext5({
        state: currentState,
        template: getTokenPriceTemplate
      });
      elizaLogger5.log("Extracting token address...");
...
// src/actions/solana/getTokenMetadata.ts
import {
...
// src/templates/tokenMetadata.ts
var getTokenMetadataTemplate = `Given the most recent message only, extract the Solana token address to fetch metadata for. This is specifically for Solana blockchain only.
...
var getTokenMetadata_default = {
  ...
  description: "Get token metadata including supply, FDV, and other details on Solana blockchain",
  handler: async (runtime, message, state, _options, callback) => {
    elizaLogger6.log(
    ...
    try {
      elizaLogger6.log("Composing token metadata context...");
      const metadataContext = composeContext6({
        state: currentState,
        template: getTokenMetadataTemplate
      });
      elizaLogger6.log("Extracting token address...");
...
var moralisPlugin = {
  ...
  actions: [
    getTokenPairs_default,
    getPairStats_default,
    getPairOHLCV_default,
    getTokenStats_default,
    getTokenPrice_default,
    getTokenMetadata_default
  ],
  evaluators: [],
  providers: []
};
var index_default = moralisPlugin;
export {
